...
//        getChildren().add(canvas);
    ...
    public final Color getBorderColor() {
        return null == borderColor ? Color.GRAY : borderColor.get();
    }
    public final void setBorderColor(final Color BORDER_COLOR) {
        ledColorProperty().set(BORDER_COLOR);
    }
    public final ObjectProperty<Color> borderColorProperty() {
        if (null == borderColor) {
            borderColor = new SimpleObjectProperty<>(this, "borderColor", Color.GRAY);
        }
        return borderColor;
    }
    ...
    private void recalc() {
        ...
        frameGradient = new LinearGradient(0.14 * size, 0.14 * size,
                                           ...
                                           new Stop(0.0, borderColor.get().deriveColor(0d, 1d, 0.77, 1d)),
    ...
    private void draw() {        
        ...
        if (isFrameVisible()) {          
            var rect1 = new Rectangle(0, 0, size, size);
            rect1.setFill(frameGradient);
            getChildren().add(rect1);
        }
        var tamanoRec = size-(0.72 * size)/2;
        var rect2 = new Rectangle(tamanoRec, tamanoRec, 0.72 * size, 0.72 * size);
        if (isOn()) {
        ...
        var desvioReflejo = size-(0.58 * size)/2;  
        var rect3 = new Rectangle(desvioReflejo, desvioReflejo, 0.58 * size, 0.58 * size);
